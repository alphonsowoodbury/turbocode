...
@click.command()
@click.option('--file', 'import_file', type=click.Path(exists=True), required=True, help='Import file path')
@click.option('--type', type=click.Choice(TYPE_CHOICES), default='all', help='Entity type to import')
...
async def _import_projects(projects_data, project_service, skip_existing):
    ...
    for project_data in projects_data:
        try:
            ...
            if skip_existing:
                try:
                    existing = await project_service.search_projects_by_name(project_data['name'])
                    if existing:
                        skipped += 1
                        continue
                except:
            ...
            project_create = ProjectCreate(
                ...
                completion_percentage=float(project_data.get('completion_percentage', 0))
            )
            ...
            console.print(f"[red]Error importing project '{project_data.get('name', 'Unknown')}': {e}[/red]")
            errors += 1
...
async def _import_issues(issues_data, issue_service, skip_existing):
    ...
    for issue_data in issues_data:
        try:
            ...
            if skip_existing:
                try:
                    existing = await issue_service.search_issues_by_title(issue_data['title'])
                    if existing:
                        skipped += 1
                        continue
                except:
            ...
            if issue_create.project_id:  # Only create if project_id is provided
                ...
                console.print(f"[yellow]Skipping issue '{issue_data['title']}' - no project_id[/yellow]")
                skipped += 1

        except Exception as e:
            console.print(f"[red]Error importing issue '{issue_data.get('title', 'Unknown')}': {e}[/red]")
            errors += 1
...
async def _import_documents(documents_data, document_service, skip_existing):
    ...
    for document_data in documents_data:
        try:
            ...
            if skip_existing:
                try:
                    existing = await document_service.search_documents(document_data['title'])
                    if existing:
                        skipped += 1
                        continue
                except:
            ...
            console.print(f"[red]Error importing document '{document_data.get('title', 'Unknown')}': {e}[/red]")
            errors += 1
...
async def _import_tags(tags_data, tag_service, skip_existing):
    ...
    for tag_data in tags_data:
        try:
            ...
            console.print(f"[red]Error importing tag '{tag_data.get('name', 'Unknown')}': {e}[/red]")
            errors += 1

...
@click.option('--format', type=click.Choice(FORMAT_CHOICES), default='json', help='Export format')
@click.option('--type', type=click.Choice(TYPE_CHOICES), default='all', help='Entity type to export')
    ...
    async def _export():
        async for session in get_db_session():
            ...
            try:
                ...
                console.print(f"[green]✓[/green] Data exported successfully to {output_path}")
    ...
    with open(file_path, 'w', newline='', encoding='utf-8') as f:
        if data:
            ...
            for item in data:
                ...
                row = {k: str(v) if hasattr(v, 'isoformat') else v for k, v in item.items()}
                writer.writerow(row)

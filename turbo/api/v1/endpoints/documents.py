...
async def export_document(
    ...
    try:
        content, content_type = await document_service.export_document(document_id, format)
...
async def duplicate_document(
    ...
    try:
        return await document_service.duplicate_document(
            document_id,
            duplicate_request.title,
            duplicate_request.version
        )
    except DocumentNotFoundError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def bulk_delete_documents(
    ...
    try:
        deleted_count = await document_service.bulk_delete_documents(bulk_request.document_ids)

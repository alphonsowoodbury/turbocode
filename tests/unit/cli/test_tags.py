...
class TestTagCLI:
    ...
    def test_create_tag_success(self, mock_get_service):
        ...
        result = self.runner.invoke(tags_group, [
            'create',
            '--name', 'test-tag',
            '--color', '#FF5733',
    ...
    def test_create_tag_duplicate_name(self, mock_get_service):
        ...
        mock_service = AsyncMock()
        mock_service.create_tag.side_effect = DuplicateResourceError("Tag", "name 'existing-tag'")
        mock_get_service.return_value = mock_service
...
class TestTagCLIValidation:
    ...
    def test_color_case_handling(self):
        ...
        result = self.runner.invoke(tags_group, [
            'create',
            '--name', 'test-tag',
            '--color', '#ff5733'  # lowercase should be accepted
        ])

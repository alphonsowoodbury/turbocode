...
import { StaffMessageBubble } from "./staff-message-bubble";
import { Button } from "@/components/ui/button";
import { MentionTextarea } from "@/components/ui/mention-textarea";
import { Card } from "@/components/ui/card";
import { Loader2, Send, Trash2, Zap } from "lucide-react";
import { useStaffConversation, useSendStaffMessage, useClearStaffConversation } from "@/hooks/use-staff";
import { useStaffStreaming } from "@/hooks/use-staff-streaming";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function StaffChat({ staffId, messagesOnly = false, inputOnly = false, enableStreaming = true }: StaffChatProps) {
  const [message, setMessage] = useState("");
  const [clearDialogOpen, setClearDialogOpen] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  // Fetch conversation history (use max limit to get all recent messages)
  const { data: conversation, isLoading, error } = useStaffConversation(staffId, { limit: 100 });
  ...
  const {
    ...
    abortStream
  } = useStaffStreaming(staffId);

  // Clear conversation mutation
  const clearConversationMutation = useClearStaffConversation();
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      if (enableStreaming) {
        ...
        toast.success("Message Sent", {
          description: "Your message has been sent. The staff member will respond shortly.",
        });
      }
    } catch (error) {
      toast.error("Error", {
  ...
  const handleClearConversation = async () => {
    try {
      await clearConversationMutation.mutateAsync(staffId);
    ...
    return (
      <form onSubmit={handleSendMessage}>
        <div className="flex gap-2 items-end">
          <MentionTextarea
            ref={textareaRef}
            placeholder="Send a message... (type @ to mention staff, Enter to send, Shift+Enter for new line)"
            value={message}
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-32">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-xs mt-1">Start a conversation with this staff member</p>
            </div>
          </div>
        ) : (
          <>
            {conversation?.messages?.map((msg) => (
              <StaffMessageBubble key={msg.id} message={msg} staffId={staffId} />
            ))}
